...
export default function BuyerSchedulePage({ params }: Props) {
      ...
      {schedule.length === 0 ? (
        <div className="py-20 text-center text-mid-gray flex flex-col items-center gap-3">
          ...
          <p className="text-body-sm">
            Once a match is scheduled, it will appear here.
          </p>
        </div>
      ) : (
        <div className="flex flex-col gap-8 print:gap-6">
          {schedule.map((day) => (
            <section key={day.date}>
              <h2 className="font-display font-semibold text-heading-4 text-ink-gray mb-3 print:text-base">
                {formatDate(day.date)}
              </h2>
              <div className="bg-white rounded-2xl border border-light-border overflow-hidden shadow-xs print:shadow-none print:rounded-none">
                            ...
                            <div className="flex items-center gap-2">
                              {counterpart.logo_url ? (
                                <img
                                  ...
                                  className="w-7 h-7 rounded-md object-contain border border-light-border bg-off-white print:hidden"
                                />
                              ) : (
                                <div className="w-7 h-7 rounded-md bg-pale-blue-tint flex items-center justify-center text-calm-blue font-bold text-xs print:hidden">
