...
export default function SellerMatchPage({ params }: { params: Promise<{ id: string }> }) {
  const { id: eventId } = use(params);
  const router = useRouter();

  useEffect(() => {
    if (window.innerWidth > 768) router.replace(`/seller/events/${eventId}/discover`);
  }, [eventId, router]);
    ...
    >
      ...
      <div className="flex items-center justify-between px-6 pt-12 pb-0 relative z-10">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border text-[11px] font-bold"
          style={{ background: "rgba(46,127,217,.12)", borderColor: "rgba(46,127,217,.25)", color: "#5BABF0" }}>
          🏷️ SELLER MODE
        </div>
      </div>
      ...
      <div className="px-6 pt-4 pb-2 relative z-10">
        ...
        <p className="text-[12px] mt-0.5" style={{ color: "rgba(255,255,255,.45)" }}>
          {hasCards ? `${counterparts.length} buyer${counterparts.length !== 1 ? "s" : ""} available` : "No more buyers to browse"}
        </p>
      </div>
      ...
      <div className="flex-1 relative flex items-center justify-center px-5 z-10">
        <div
          ...
          style={{ opacity: 0, color: "#00D4FF", borderColor: "#00D4FF", background: "rgba(0,212,255,.12)", boxShadow: "0 0 20px rgba(0,212,255,.30)" }}
        >
          ↓ MATCH
        </div>
        ...
        {!hasCards ? (
          <div className="flex flex-col items-center gap-4 text-center px-8">
            ...
            <p className="text-[13px]" style={{ color: "rgba(255,255,255,.45)" }}>Check back when new buyers are added.</p>
          </div>
        ) : (
          <>
              ...
              <>
                ...
                {overlayProducts.length === 0 && overlayCatalogues.length === 0 && (
                  <div className="flex flex-col items-center gap-3 py-10 text-center">
                    ...
                    <p className="text-[12px]" style={{ color: "rgba(255,255,255,.3)" }}>This buyer hasn&apos;t selected products for this event.</p>
                  </div>
                )}
                {current?.bio && (
                  <>
                ...
                >
                  {requesting ? <Loader2 size={16} className="animate-spin" /> : `↓ Match with ${current?.company_name ?? "Buyer"}`}
                </button>
              </>
            )}
          </div>
        ...
        {selectedProduct && (
          <>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5" style={{ WebkitOverflowScrolling: "touch" }}>
              ...
              >
                {requesting ? <Loader2 size={16} className="animate-spin" /> : "Request Match with Buyer"}
              </button>
            </div>
          </>
        )}
      ...
      >
        ...
        <div className="flex flex-col gap-2.5 w-[85%] relative z-10">
          <button
            onClick={() => { setScreen("deck"); router.push(`/seller/events/${eventId}/matches`); }}
  ...
  return (
    <>
      ...
      <div className="px-5 py-4">
        <p className="text-[10px] font-semibold uppercase tracking-[.06em] mb-2" style={{ color: "rgba(255,255,255,.45)" }}>Buyer</p>
...
function Confetti() {
  ...
  const particles = Array.from({ length: 30 }, (_, i) => ({
    ...
    size: (i % 3) * 3 + 4, left: (i * 3.33) % 100, top: (i * 7) % 30 + 5,
    delay: i * 0.027, dur: (i % 3) * 0.6 + 1.6, circle: i % 2 === 0,
  }));
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      {particles.map((p) => (
        <div key={p.id} style={{
          ...
          left: `${p.left}%`, top: `${p.top}%`, borderRadius: p.circle ? "50%" : "3px",
          animation: `confetti-fall ${p.dur}s ease-in ${p.delay}s forwards`, opacity: 0,
        }} />
      ))}
    </div>
  );
